...
// MODIFIED: Added Trash2 icon
import { Bot, Send, Loader2, Baby, Utensils, Clock, Heart, MessageSquare, ThumbsUp, Users, BarChart3, Copy, Trash2 } from "lucide-react";
...
export default function NeonestAi() {
  ...
  }, [role, token, chatHistory, setChatHistory, historyLoaded]); // MODIFIED: Added historyLoaded to dependency array
  ...
  // ADDED: Function to handle clearing the chat history
  const handleClearChat = async () => {
    try {
      setChatHistory(role, []);
      await saveChatHistory(role, [], token);
    } catch (error) {
      console.error("Failed to clear chat history:", error);
      alert("Could not clear the chat. Please try again.");
    } finally {
      setShowClearConfirm(false);
      localStorage.removeItem("chatHistory"); // or your specific key
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 space-y-10">
      <Card className="max-w-4xl mx-auto">
        <CardHeader className="flex flex-row justify-between items-center bg-pink-100 rounded-t-lg px-6 py-4">
          <div className="flex items-center gap-3">
            <Bot className="w-6 h-6 text-pink-500" />
            <CardTitle>NeoNest AI Chatbot</CardTitle>
          </div>
          {/* MODIFIED: Wrapped controls in a div for better layout */}
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  className="px-3 h-8 text-sm text-gray-600 hover:bg-pink-200 hover:text-red-500"
        ...
        <CardContent className="space-y-6 p-6 relative">
          ...
          {showClearConfirm && (
            <div className="absolute inset-0 bg-black bg-opacity-40 flex justify-center items-center z-20 rounded-lg backdrop-blur-sm">
              <div className="bg-white p-6 rounded-lg shadow-xl text-center space-y-4 w-full max-w-sm mx-4">
                <h3 className="text-lg font-semibold text-gray-800">Clear Chat History</h3>
               <p className="text-sm text-gray-600">
             Are you sure you want to delete all messages for the &apos;{roles.find(r => r.value === role)?.label}&apos; role? This cannot be undone.
            </p>
                <div className="flex justify-center gap-4 pt-2">
                  <Button variant="outline" onClick={() => setShowClearConfirm(false)}>
                    Cancel
                  </Button>
                  <Button className="bg-red-500 hover:bg-red-600 text-white" onClick={handleClearChat}>
                    Yes, Clear It
                  </Button>
                </div>
              </div>
                ...
                {[1, 2, 3].map((_, i) => (
                  <div key={i} className={`flex ${i % 2 === 0 ? "justify-start" : "justify-end"} animate-pulse`}>
                    <div className={`rounded-xl px-4 py-3 min-w-[60%] ${i % 2 === 0 ? "bg-gray-200" : "bg-gradient-to-r from-pink-300 to-purple-300"}`}>
                      <div className="h-4 bg-gray-300/50 rounded w-3/4 mb-2" />
          ...
          {showNewMessageButton && messages.length > 0 && ( // MODIFIED: Ensure button doesn't show on empty chat

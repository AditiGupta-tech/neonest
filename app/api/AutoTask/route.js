...
export async function POST(req) {
  ...
  try {
    // Move GoogleGenerativeAI initialization inside the function
    ...
    if (!message || message.length < 9)
      return NextResponse.json({ ...errorMessage, actionName: "Too Few Information" });

    const agent = new JSONAgent({ model: genAi });
    const date = new Date();
    const prompt = `${message}. The date is ${date.toUTCString()} and time is ${
      time ? time : date.toTimeString()
    }.`;
    ...
    if (!user || !userExists || !user?.user?.id) {
      return NextResponse.json({ ...errorMessage, actionName: "Authentication Failed" }, { status: 401 });
    }
    ...
    if (Array.isArray(agent_reply)) {
      for (let task of agent_reply) {
        if (task) replyMessage.push(await saveData(task));
        else replyMessage.push({ isAction: false, actionName: task?.actionName || "invalid request", request: task?.request || "null" });
      }
    } else {
    ...
    return NextResponse.json({ ...errorMessage, actionName: "Invalid request" });
  }
}
